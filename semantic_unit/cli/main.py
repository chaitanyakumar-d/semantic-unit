...
def evaluate(
    ...
    model: str = typer.Option("gpt-4o-mini", "--model", "-m", help="LLM model to use for evaluation"),
    temperature: float = typer.Option(0.0, "--temperature", "-t", help="Sampling temperature (0.0 for deterministic)"),
    ...
    try:
        ...
        if json_output:
            ...
            table.add_row("Actual", result.actual[:100] + "..." if len(result.actual) > 100 else result.actual)
            table.add_row("Expected", result.expected[:100] + "..." if len(result.expected) > 100 else result.expected)
