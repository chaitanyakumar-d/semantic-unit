...
class TestSemanticJudge:
    ...
    def test_evaluate_success(self, mock_completion):
        ...
        mock_response.choices[0].message.content = '{"score": 0.95, "reasoning": "High semantic alignment observed"}'
        mock_response.usage.total_tokens = 150
        mock_completion.return_value = mock_response
        
        judge = SemanticJudge()
        result = judge.evaluate(
            actual="The test passed successfully",
            expected="The test was successful"
        )
    ...
    def test_evaluate_with_metadata(self, mock_completion):
        ...
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = '{"score": 0.88, "reasoning": "Minor drift detected"}'
